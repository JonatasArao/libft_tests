...
MU_TEST(test_ft_striteri_s1_null)
{
	...
	// ACT & ASSERT
	actual_result = 1;
	expected_result = capture_segfault_ft_striteri(&ft_striteri, NULL, &toggle_case);
...
MU_TEST(test_ft_striteri_s2_null)
{
	...
	// ACT & ASSERT
	actual_result = 1;
	expected_result = capture_segfault_ft_striteri(&ft_striteri, "42", NULL);
	mu_assert(expected_result == actual_result, "Expected segmentation fault, but it did not occur.");
}
...
MU_TEST(test_ft_striteri_s1_null_s2_null)
{
	...
	// ACT & ASSERT
	actual_result = 1;
	expected_result = capture_segfault_ft_striteri(&ft_striteri, NULL, NULL);
	mu_assert(expected_result == actual_result, "Expected segmentation fault, but it did not occur.");
}
